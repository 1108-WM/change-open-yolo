...
- 跨视角二维掩码证据图初版代码已经接入。只保留多视角掩码簇的版本召回太低；加入单视角孤立候选的版本在四十八场景划分上有正向，但在九十六场景划分上没有保持，因此暂不替代当前最佳。
