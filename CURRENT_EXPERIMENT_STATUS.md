...
- `tools/analyze_superpoint_candidate_diagnostics.py`
  - 新增最大连通分量分支的点数、连通、覆盖、冲突重叠和保留比例汇总。
  - `--focus` 格式错误时改为忽略并报告，不再直接报错。
  - `non_single_core_boundary_candidates` 不再混入无可靠核心候选，另存 `missing_core_boundary_candidates`。
...
- 候选目录：
  - `/tmp/mask_graph_proposals_scannet200_superpoint_largest_cc_diag_10scenes`
- 汇总 JSON：
  - `/tmp/superpoint_largest_cc_diag_10scenes_summary.json`
- 总观测数：`663`
- 导出候选数：`63`
- 支持边：`857`
- 弱边：`1037`
...
- `scene0011_00 / dishwasher`
  - 点级：`435` 点，`11` 个连通分量。
  - 严格核心+边界：`502` 点，`3` 个连通分量。
  - 最大点级连通分量：`500` 点，`1` 个连通分量。
  - 保留比例：`0.996`，覆盖点级候选比例：`0.487`，被点级候选覆盖比例：`0.424`，冲突重叠：`0.205`。
- `scene0046_02 / toilet`
  - 点级：`823` 点，`15` 个连通分量。
  - 严格核心+边界：`1062` 点，`3` 个连通分量。
  - 最大点级连通分量：`1048` 点，`1` 个连通分量。
  - 保留比例：`0.987`，覆盖点级候选比例：`0.798`，被点级候选覆盖比例：`0.627`，冲突重叠：`0.166`。
- `scene0046_02 / door`
  - 点级：`540` 点，`2` 个连通分量。
  - 严格核心+边界：`1946` 点，`5` 个连通分量。
  - 最大点级连通分量：`1871` 点，`1` 个连通分量。
  - 保留比例：`0.961`，覆盖点级候选比例：`0.917`，被点级候选覆盖比例：`0.265`，冲突重叠：`0.033`。

当前判断：

- 最大点级连通分量裁剪几乎不改变平均点数：`1072.6 -> 1069.7`，说明被移除的主要是小碎片。
- 单连通候选从严格核心+边界的 `47 / 63` 提升到 `60 / 63`，碎片清理效果明确。
- 平均冲突重叠没有下降：仍为 `0.074`，说明这一步主要解决碎片，不解决与冲突超点的重叠。
- `door` 个例仍明显大于点级候选，且被点级候选覆盖比例只有 `0.265`，后续若进入候选融合，仍需要结合 Mask3D 覆盖和类别/边界证据再决定新增、补全或拒绝。
- 当前仍不建议直接跑最终 AP；下一步更稳的是人工查看重点个例或扩到更大诊断集，确认最大连通分量裁剪不会删掉真实主体。
