...
## 2026-09-07 当前最高优先级状态

旧版 FI1-D-v3 + duplicate-safe DM-SMS-1 的 val312 端到端开放词汇结果已判定失败：DM-SMS-1 虽将同合同控制从 `19.458571` 提升到 `22.460682 AP`，但仍低于 OpenYOLO3D 论文 `24.7 AP` 和服务器历史完整控制 `25.415455 AP`。根因不是 evaluator，而是旧接口把 `213,233` 个原始语义成员折叠为 `39,198` 个几何代表，并把类别无关 FI1 几何质量分数用于类别相关排序。

新的独立分支为 `fi1-d-v3-geometry-semantic-separation-20260907`：

- S0/S1 提交 `8495b950bfaa3be39788c098f882f18a230b2799`：建立 `39,250` 个几何证据记录与 `213,339` 条语义假设，独立审计 `error_count=0`；
- S2 提交 `5d423b346336a5439feb672a0225580738097b96`：无 GT 重建历史 B0 控制缓存，`213,227` 列与产生 `25.415455` 的冻结权威路径在 312 场逐值一致，独立审计 `error_count=0`；
- S0–S2 均未运行新的 Alpha/SAM、DINO、Qwen、val312 GT 或 AP；大型输出只存在于远程服务器，不在 Git 仓库中。

下一阶段 S3 尚未实现。不能继续仅凭同一个 official100 的总体 AP 选型：历史 Z3 五折增量为 `+3.1695/+1.0575/+4.1597/+3.3377/-3.2307`，总体提升掩盖了严重折间不稳定；official100 还已被大量历史方案反复用于选择，存在研究者层面的验证集过拟合。S3 应先冻结迁移风险合同，优先增加一个从未参与历史开发的 ScanNet200 train-split lockbox；若无法建立，只能把 official100 结果视为开发证据，而非迁移证明。
