...
- 当前最好 `even48`：平均精度 `0.272610`，百分之五十重叠率平均精度 `0.345769`，百分之二十五重叠率平均精度 `0.389491`。
- 当前最好 `even96`：平均精度 `0.273030`，百分之五十重叠率平均精度 `0.340244`，百分之二十五重叠率平均精度 `0.383687`。
- ScanNet200 当前评估是 Top-K 开放评测，不是 Top-1 硬单分类。
...
1. **超点优先的候选生成**
   ...
   不再先做点级反投影、再用超点修补，而是先把二维检测框、二维掩码、深度一致性和多视角支持投票到超点上，再组合相邻高支持超点形成候选。

   目标：让超点成为候选生成的基本单位，减少点级反投影带来的背景和平面污染。

2. **真正的多视角候选聚合**

   当前代码更多是在统计多视角支持，还不是 `Details Matter` 式的跟踪聚合。下一步应把不同视角中指向同一三维区域的二维掩码关联起来，再合并成一个三维候选。

   目标：让候选在生成阶段就更接近“完整、干净、单一物体”，而不是生成后再删除坏候选。

3. **把质量控制前移到候选生成阶段**

   二维掩码内外支持、深度一致性、多视角重复出现、大平面判断、类别一致性，不应只在候选生成后做过滤，而应参与候选生成本身。

   目标：少生成坏候选，而不是生成大量坏候选后再筛。

4. **加入 Grounded SAM 作为新的二维候选来源**

   当前项目可以尝试 Grounded SAM，但建议作为新来源接入，不要一开始直接替换现有 Segment Anything 模型流程。

   当前流程是：

   ```text
   YOLO-World 给二维框
   -> Segment Anything 模型根据框生成二维掩码
   -> 掩码反投影到三维
   ```
   ...
   ```text
   A. 当前 YOLO-World + Segment Anything 模型
   B. Grounded SAM only
   C. YOLO-World + Segment Anything 模型 + Grounded SAM 融合
   ```

   接入方式：导出和现有 `sam_fused` 候选兼容的候选文件，保留 `sam_mask_path`、类别、分数、种子点、支持视角等字段，然后让 `utils/backprojection_fusion.py` 直接消费。融合时可把来源名设为 `grounded_sam`，再单独设置来源优先级和每场景候选数上限。

   判断标准：先跑候选诊断，不要直接只看最终平均精度。如果 Grounded SAM 的真正补全候选更多、背景坏候选更少，再进入完整 AP 评估。

5. **Alpha-CLIP 重新定位为后续语义模块**

   Alpha-CLIP 不适合作为当前低质量候选上的简单后置改类器。更合理的用法是：等候选质量提高后，把它作为最终语义分类和低可信语义过滤模块。

   目标：复现 `Details Matter` 的角色分工，即候选生成负责形状和召回，Alpha-CLIP 负责最终类别，标准化相似度负责过滤低可信候选。

6. **学习式候选排序**

   现有几何、深度、二维掩码支持、多视角支持和层级特征有诊断价值，但不适合硬删除。下一步可以用于候选排序、分数校准和每类候选数分配。

   目标：选择更值得加入的候选，而不是简单删除一批候选。
...
- Grounded SAM 新来源：参考 `tools/export_sam_fused_proposals.py`，新增一个候选导出脚本，输出格式对齐现有 `sam_fused` 候选。
