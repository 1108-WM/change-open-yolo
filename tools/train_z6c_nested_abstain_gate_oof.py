#!/usr/bin/env python3
"""Train nested-cross-fitted Z6c accept/abstain gates without threshold scans."""
...
def _groups(rows: list[dict], indexes: np.ndarray) -> list[np.ndarray]:
    ...
    return [np.asarray(grouped[key], dtype=np.int64) for key in order]


def _choose(rows: list[dict], indexes: np.ndarray, scores: np.ndarray) -> tuple[int, int | None, float]:
    ordered = sorted(
        (int(index) for index in indexes),
        key=lambda index: (
            -float(scores[index]), -int(bool(rows[index]["option_is_current"])),
            int(rows[index]["option_class_index"]),
        ),
    )
    selected = ordered[0]
    current = next((index for index in ordered if bool(rows[index]["option_is_current"])), None)
    runner = float(scores[ordered[1]]) if len(ordered) > 1 else 0.0
    return selected, current, float(scores[selected] - runner)


def _gate_feature(
    features: np.ndarray, scores: np.ndarray, selected: int, current: int | None, margin: float
) -> np.ndarray:
    current_score = float(scores[current]) if current is not None else 0.0
    selected_row = features[selected]
    # Selector confidence plus the selected option's class-agnostic evidence.
    return np.asarray([
...
def _base_model(seed: int) -> HistGradientBoostingRegressor:
    return HistGradientBoostingRegressor(
    ...
    for outer in outer_specs:
        ...
        for group_name, columns in GROUPS.items():
            ...
            for inner in sorted(split["folds"], key=lambda row: int(row["fold_index"])):
                ...
                inner_fold_count += 1
                print(
                    f"[Z6c nested gate] outer={outer_index} model={group_name} "
                    f"inner={inner['fold_index']} complete",
                    flush=True,
                )
                del model
                gc.collect()
            ...
            for indexes in _groups(rows, outer_validation):
                ...
                proposal = (indexes, selected, current, margin, first)
            ...
            pending_gate_predictions = iter(
                gate.predict(np.asarray(pending_gate_features, dtype=np.float32)).tolist()
            ...
            for indexes, selected, current, margin, first in validation_proposals:
                if current is not None and selected == current:
                    ...
                    predicted_delta = float(next(pending_gate_predictions))
                ...
                gated_outputs[group_name][key] = {
                    ...
                    "gate_predicted_delta": predicted_delta,
                    "gate_state": state,
            ...
            fold_payload["models"][group_name] = {
                ...
                "gate_target_positive_count": int((gate_targets > 0).sum()),
                "gate_target_negative_count": int((gate_targets < 0).sum()),
                "gate_target_zero_count": int((gate_targets == 0).sum()),
        ...
        print(f"[Z6c nested gate] outer fold {outer_index} complete", flush=True)
    ...
    with (args.output_dir / "oof_selections.jsonl").open("w") as handle:
        for key in prediction_keys:
            ...
            handle.write(json.dumps({
                ...
                "selectors": {
                    f"gated_{name}": gated_outputs[name][key] for name in GROUPS
                },
            }, ensure_ascii=False, sort_keys=True) + "\n")
    summary_counts = {}
    for name in GROUPS:
        states = Counter(row["gate_state"] for row in gated_outputs[name].values())
        summary_counts[f"gated_{name}"] = dict(states)
    summary = {
        "diagnostic_type": "official100 nested-cross-fitted Z6c accept/abstain gate",
        ...
        "gate_state_counts": summary_counts, "folds": fold_summaries,
        "acceptance_contract": "accept proposed new class iff nested-OOF predicted AP-quality delta > 0; otherwise keep current",
        "base_selector_contract": "same frozen semantic-only and semantic+DINO selector models; no parameter changes",
        "gate_model_contract": {
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("docs/diagnostics/z6c_nested_abstain_gate_oof_official100_20260812"))
