...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/output/scannet200/subset_sweeps/even48_cc_keep_ratio_sweep}"
MODE="${MODE:-all}"  # k050 | k060 | k070 | all
...
case "$MODE" in
  k050)
    run_eval "cc_keep050" "3-6 min on RTX 4090" --backprojection_cc_min_keep_ratio 0.50
    ;;
  k060)
    run_eval "cc_keep060" "3-6 min on RTX 4090" --backprojection_cc_min_keep_ratio 0.60
    ;;
  k070)
    run_eval "cc_keep070" "3-6 min on RTX 4090" --backprojection_cc_min_keep_ratio 0.70
    ;;
  all)
    run_eval "cc_keep050" "3-6 min on RTX 4090" --backprojection_cc_min_keep_ratio 0.50
    run_eval "cc_keep060" "3-6 min on RTX 4090" --backprojection_cc_min_keep_ratio 0.60
...
echo "[DONE] Wrote even48 CC keep-ratio sweep outputs to $OUT_DIR"
