...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/output/scannet200/subset_sweeps/even48_quality_guard}"
MODE="${MODE:-all}"  # current | qz050 | qz075 | cc_clean | qz075_cc | all
...
case "$MODE" in
  current)
    run_eval "current" "3-6 min on RTX 4090"
    ;;
  qz050)
    run_eval "qz050" "3-6 min on RTX 4090" \
      --backprojection_min_scene_source_quality_z 0.50
    ;;
  qz075)
    run_eval "qz075_q014" "3-6 min on RTX 4090" \
      --backprojection_min_quality_score 0.14 \
      --backprojection_min_scene_source_quality_z 0.75
    ;;
  cc_clean)
    run_eval "cc_clean" "3-7 min on RTX 4090/CPU" \
    ...
    ;;
  qz075_cc)
    run_eval "qz075_q014_cc_clean" "3-7 min on RTX 4090/CPU" \
      --backprojection_min_quality_score 0.14 \
      --backprojection_min_scene_source_quality_z 0.75 \
      --backprojection_cc_cleanup \
      --backprojection_cc_radius 0.03 \
...
echo "[DONE] Wrote quality-guard outputs to $OUT_DIR"
