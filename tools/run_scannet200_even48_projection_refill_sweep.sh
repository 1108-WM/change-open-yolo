...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/output/scannet200/subset_sweeps/even48_projection_refill}"
...
sources.update(item.get("source_kind") for item in applied)
print(
    f"[REPORT] {name}: applied={len(applied)} "
    f"sources={dict(sources)} skipped={skipped.most_common(6)}"
)
PY
}
...
run_eval "proj_refill_m24_s18_b6" "3-6 min on RTX 4090" \
  --backprojection_max_candidates_per_scene 24 \
  --backprojection_source_max_candidates sam_fused=18,bpr=6

run_eval "proj_refill_m30_s24_b6" "3-6 min on RTX 4090" \
  --backprojection_max_candidates_per_scene 30 \
  --backprojection_source_max_candidates sam_fused=24,bpr=6

echo "[DONE] Wrote projection-refill sweep outputs to $OUT_DIR"
