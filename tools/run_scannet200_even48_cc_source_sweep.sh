...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/output/scannet200/subset_sweeps/even48_cc_source_sweep}"
MODE="${MODE:-all}"  # bpr | sam | keep2 | all
...
case "$MODE" in
  bpr)
    run_eval "cc_bpr_only" "3-6 min on RTX 4090" \
      --backprojection_cc_keep_topk 1 \
      --backprojection_cc_source_kinds bpr
    ;;
  sam)
    run_eval "cc_sam_only" "3-6 min on RTX 4090" \
      --backprojection_cc_keep_topk 1 \
      --backprojection_cc_source_kinds sam_fused
    ;;
  keep2)
    run_eval "cc_all_keep2" "3-6 min on RTX 4090" \
      --backprojection_cc_keep_topk 2
    ;;
  all)
    run_eval "cc_bpr_only" "3-6 min on RTX 4090" \
      --backprojection_cc_keep_topk 1 \
...
echo "[DONE] Wrote even48 CC source sweep outputs to $OUT_DIR"
