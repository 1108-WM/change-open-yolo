...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/output/scannet200/subset_sweeps/even48_cc_geometry_sweep}"
MODE="${MODE:-all}"  # r020 | r040 | r050 | r050m30 | r070 | all
...
case "$MODE" in
  r020)
    run_eval "cc_r020_m50" "3-6 min on RTX 4090" \
      --backprojection_cc_radius 0.02 \
      --backprojection_cc_min_component_points 50
    ;;
  r040)
    run_eval "cc_r040_m50" "3-6 min on RTX 4090" \
      --backprojection_cc_radius 0.04 \
      --backprojection_cc_min_component_points 50
    ;;
  r050)
    run_eval "cc_r050_m50" "3-6 min on RTX 4090" \
      --backprojection_cc_radius 0.05 \
      --backprojection_cc_min_component_points 50
    ;;
  r050m30)
    run_eval "cc_r050_m30" "3-6 min on RTX 4090" \
      --backprojection_cc_radius 0.05 \
      --backprojection_cc_min_component_points 30
    ;;
  r070)
    run_eval "cc_r070_m50" "3-6 min on RTX 4090" \
      --backprojection_cc_radius 0.07 \
      --backprojection_cc_min_component_points 50
    ;;
  all)
    run_eval "cc_r020_m50" "3-6 min on RTX 4090" \
...
echo "[DONE] Wrote even48 CC geometry sweep outputs to $OUT_DIR"
