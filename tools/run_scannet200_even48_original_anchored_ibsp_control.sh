...
# C' 受控评测：历史强 SAM-fused + BPR 基线仅替换为原始超点约束的 f30 IBSp。
# 不重跑原始超点 B，避免无关的重复计算。
...
IBSP_ROOT="${IBSP_ROOT:-$ROOT_DIR/output/original_anchored_ibsp_dense_even48_f30}"
OUT_DIR="${OUT_DIR:-$ROOT_DIR/output/scannet200/original_anchored_ibsp_control_even48_20260722}"
...
for scene in (line.strip() for line in scene_list.read_text().splitlines()):
    ...
    refined = np.load(ibsp_root / scene / f"{scene_id}.npy", mmap_mode="r")
    if original.shape != refined.shape:
        raise SystemExit(f"Shape mismatch for {scene}: {original.shape} vs {refined.shape}")
    if not np.array_equal(original[:, :9], refined[:, :9]) or not np.array_equal(original[:, 10:], refined[:, 10:]):
        raise SystemExit(f"Non-superpoint columns changed for {scene}")
    changed_scenes += int(np.any(original[:, 9] != refined[:, 9]))
if changed_scenes == 0:
    raise SystemExit("Original-anchored IBSp did not change any scene's superpoint ids")
print(
    "[CHECK] Original-anchored IBSp differs only in column 9; "
    f"changed scenes: {changed_scenes}, unchanged scenes: {48 - changed_scenes}"
)
PY

echo "[RUN] C_prime_original_anchored_f30_ibsp"
"$PYTHON" run_evaluation.py \
  ...
  --backprojection_report_path "$OUT_DIR/reports/C_prime_original_anchored_f30_ibsp.json" \
  --eval_output_file "$OUT_DIR/C_prime_original_anchored_f30_ibsp.csv" \
  >"$OUT_DIR/C_prime_original_anchored_f30_ibsp.log" 2>&1

"$PYTHON" - "$OUT_DIR/C_prime_original_anchored_f30_ibsp.csv" <<'PY'
import csv
import math
import sys
...
print("[RESULT] C_prime_original_anchored_f30_ibsp: " + " ".join(
