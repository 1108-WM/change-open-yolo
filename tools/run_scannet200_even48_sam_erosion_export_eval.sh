...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/output/scannet200/subset_sweeps/even48_sam_erosion}"
...
export_sam() {
  local name="$1"
  local erode_pixels="$2"
  local export_dir="$ROOT_DIR/output/sam_fused_proposals_scannet200_even48_erode${erode_pixels}"
...
case "$MODE" in
  erode5)
    sam_dir="$(export_sam sam_erode5 5)"
    run_eval sam_erode5 "$sam_dir"
    ;;
  erode9)
    sam_dir="$(export_sam sam_erode9 9)"
    run_eval sam_erode9 "$sam_dir"
    ;;
  all)
    sam_dir="$(export_sam sam_erode5 5)"
    run_eval sam_erode5 "$sam_dir"
    ...
    ;;
  *)
    echo "Unknown MODE=$MODE; expected erode5, erode9, or all" >&2
    exit 2
    ;;
esac

echo "[DONE] Wrote SAM erosion outputs to $OUT_DIR"
