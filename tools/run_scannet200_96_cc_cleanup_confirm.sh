...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/output/scannet200/subset_sweeps/cc_cleanup_96_confirm}"
MODE="${MODE:-all}"  # even96 | odd96 | all
CONFIG_NAME="${CONFIG_NAME:-cc_clean}"
CC_MIN_KEEP_RATIO="${CC_MIN_KEEP_RATIO:-0.0}"
CC_KEEP_RATIO_SCORE_WEIGHT="${CC_KEEP_RATIO_SCORE_WEIGHT:-0.0}"
...
EVEN96_LIST="${EVEN96_LIST:-$ROOT_DIR/output/scannet200/scene_splits/even96.txt}"
ODD96_LIST="${ODD96_LIST:-$ROOT_DIR/output/scannet200/scene_splits/odd96.txt}"
...
if [[ "$CC_MIN_KEEP_RATIO" != "0" && "$CC_MIN_KEEP_RATIO" != "0.0" ]]; then
  CC_CLEAN_ARGS+=(--backprojection_cc_min_keep_ratio "$CC_MIN_KEEP_RATIO")
fi

if [[ "$CC_KEEP_RATIO_SCORE_WEIGHT" != "0" && "$CC_KEEP_RATIO_SCORE_WEIGHT" != "0.0" ]]; then
  CC_CLEAN_ARGS+=(--backprojection_cc_keep_ratio_score_weight "$CC_KEEP_RATIO_SCORE_WEIGHT")
fi
...
run_split() {
  ...
  local scene_list="$2"
  local eta="$3"
  local name="${split_name}_${CONFIG_NAME}"
...
case "$MODE" in
  even96)
    run_split even96 "$EVEN96_LIST" "6-10 min on RTX 4090"
    ;;
  odd96)
    run_split odd96 "$ODD96_LIST" "6-10 min on RTX 4090"
    ;;
  all)
    run_split even96 "$EVEN96_LIST" "6-10 min on RTX 4090"
    run_split odd96 "$ODD96_LIST" "6-10 min on RTX 4090"
    ;;
  *)
    echo "Unknown MODE=$MODE. Use even96, odd96, or all." >&2
    exit 2
    ;;
esac

echo "[DONE] Wrote CC-cleanup 96-scene confirmation outputs to $OUT_DIR"
