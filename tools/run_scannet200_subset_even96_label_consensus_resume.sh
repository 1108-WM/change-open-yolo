...
run_eval sp_cov030_x3_siou060_sqs_labelcal "6-10 min on RTX 4090; on-the-fly label consensus adds CPU projection work" \
  "${BEST_ARGS[@]}" \
  "${SQS_ARGS[@]}" \
...
run_eval sp_cov030_x3_siou060_sqs_labelcal_softfilter "6-10 min on RTX 4090; on-the-fly label consensus adds CPU projection work" \
  "${BEST_ARGS[@]}" \
  "${SQS_ARGS[@]}" \
  --backprojection_label_consensus_calibration_weight 0.35 \
...
echo "[DONE] Resumed even96 label-consensus sweep outputs in $OUT_DIR"
