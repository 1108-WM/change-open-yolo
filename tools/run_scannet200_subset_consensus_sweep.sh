...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/output/scannet200/subset_sweeps/even96_consensus}"
...
run_eval sp_min_views2 \
  "${BEST_SP_ARGS[@]}" \
  --backprojection_min_support_views 2

run_eval sp_min_views3 \
  "${BEST_SP_ARGS[@]}" \
  --backprojection_min_support_views 3

run_eval sp_mean_iou030 \
  "${BEST_SP_ARGS[@]}" \
  --backprojection_min_support_mean_iou 0.30

run_eval sp_views2_mean_iou030 \
  "${BEST_SP_ARGS[@]}" \
  --backprojection_min_support_views 2 \
  --backprojection_min_support_mean_iou 0.30

echo "[DONE] Wrote consensus sweep outputs to $OUT_DIR"
